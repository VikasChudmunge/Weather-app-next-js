...
export default function Home() {
  ...
  const getWeather = async () => {
    const api_key = '8ef74bddf3c244bda4483949232610';
    ...
    if (location) {
      try {
        ...
        if (data) {
          ...
          const tempCelsius = ((data.current.temp_f - 32) * 5) / 9;
          let result = Math.ceil(tempCelsius);
          setWeather( 
          <>
              ...
              <div className='float-left text-6xl degrees'>{result}</div>
                </div>
              </div>
              ...
              <div className='flow-root p-2 '>
                ...
                <div className='float-right text-gray-500'>Wind: {api_data.wind} mph </div>
                <div className='float-left text-gray-500'>Visibility: {api_data.visibility} mi</div>
  ...
  return (
    <>
      <nav className='flex items-center justify-center bg-gray-300 py-4 w-full m-0 opacity-98'>
        <div className='relative'>
          ...
          <input className='block bg-slate-800 text-white rounded-lg opacity-70 pl-10 p-4'
            type='search' id='location'
            placeholder='Enter Location Here'
            value={location}
            onChange={(e)=> setLocation(e.target.value)}
          />
        </div>
        <button onClick={getWeather} className='bg-blue-600 hover:bg-blue-800 text-white font-bold m-2 p-2.5 rounded-lg' id='search'>
